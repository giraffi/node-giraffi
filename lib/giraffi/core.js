...
Retrieve.prototype.perform = function (callback) {
  ...
  if (this.message instanceof Object) {
    ...
    requestObj = {
      ...
      level : this._level || null,
      limit : this._limit || 10 // Default limit to retrieve : 10
    };
    queryString = qstring.stringify(requestObj);
  }

  var self = this, retrieveOptions = {
    uri: this.config.giraffiUrl + '?apikey=' + this.config.apikey + '&' + queryString,
    method: 'GET',
    headers: {
      'Content-Type': 'application/json'
    }
  ...
  common.giraffi(retrieveOptions, this.callback, function (res, body) {
